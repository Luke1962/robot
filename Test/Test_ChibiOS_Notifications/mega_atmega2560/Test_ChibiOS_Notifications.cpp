...
static msg_t Thread1(void *arg) {
	...
	Serial.print("sizeof(listener)");		Serial.println(sizeof(listener));
	while (TRUE) {
		chEvtWaitAny(ALL_EVENTS );
		while ((d = listener.get()) != NULL) {

			if (d->set)
				digitalWrite(d->pin, 1);//palSetPad(GPIOD, d->pin);
			else
				digitalWrite(d->pin, 0);//palClearPad(GPIOD, d->pin);
	...
	while (TRUE) {
		...
		if (0x01 & evt) {
			while ((d = listener1.get()) != NULL) {
				...
				if (d->set){
					//	chprintf((BaseSequentialStream *) &SD2,	"%6d palSetPad(GPIOD, %d);\r\n", chTimeNow(),d->pin);
					Serial.print("Set pin ;");		Serial.println(d->pin);
				}
				else
					//chprintf((BaseSequentialStream *) &SD2,"%6d palClearPad(GPIOD, %d);\r\n", chTimeNow(),d->pin);
					Serial.print("Reset pin ;");		Serial.println(d->pin);
				listener1.release(d);
			}
		}
		...
		if (0x02 & evt) {
			while ((d = listener2.get()) != NULL) {
				...
				if (d->set){
					//chprintf((BaseSequentialStream *) &SD2,"%6d palSetPad(GPIOD, %d);\r\n", chTimeNow(),	d->pin);
					Serial.print("listener2 Set pin ;");		Serial.println(d->pin);

				}
				else
					//chprintf((BaseSequentialStream *) &SD2,	"%6d palClearPad(GPIOD, %d);\r\n", chTimeNow(),	d->pin);
					Serial.print("listener2 Clear pin ;");		Serial.println(d->pin);
