...
#define CAMERASERIALPORT     Serial3
...
void clearRxBuf()
{
	while (CAMERASERIALPORT.available())
	{
		CAMERASERIALPORT.read();
	}
}
/*********************************************************************/
...
void sendCmdWaitAck(byte c0, byte c1, byte c2, byte c3, byte c4, byte c5)
{
	...
	while (1)
	{
		clearRxBuf();
		sendCmd(cmd, 6);
		if (CAMERASERIALPORT.readBytes((char *)resp, 6) != 6) continue;
...
void cameraSYNC()
{
	...
	CAMERASERIALPORT.setTimeout(500);
	while (1)
	{
		// pulisce il buffer -----
		clearRxBuf();
		...
		// attendi 6 caratteri nel buffer ----------------------------------
		while (CAMERASERIALPORT.available() <= 6) { delayMicroseconds(10); }

		// leggi i 6 caratteri e mettili in resp[]----------------------
		Serial.print("<");
		for (size_t i = 0; i < 6; i++) {
			resp[i] = CAMERASERIALPORT.read();
			Serial.print(resp[i], HEX); Serial.print(" ");
		}
		Serial.print(">");

		// verifica che la risposta sia un ACK --------------------------------------------------
		// se sì esci dal loop
...
void SerialCamera_Sync()
{
	...
	while (1)
	{
		//clearRxBuf();
		sendCmd(cmd, 6);
		if (CAMERASERIALPORT.readBytes((char *)resp, 6) != 6)
		{
			continue;
		}
...
void SerialCamera_Initial(byte c0, byte c1, byte c2, byte imagFormat, byte rawSetting, byte jpegResolution)
{
	dbg("\r\nInitial...")
		char cmd[6];
	cmd[0] = CMD_PREFIX;
	...
	CAMERASERIALPORT.setTimeout(100);
	while (1)
	{
		clearRxBuf();
		sendCmd(cmd, 6);
...
// solo per jpeg
void SerialCamera_SetPktSize(unsigned int packageSize)
{
...
void Capture_orig()
{
	...
	while (1)
	{
		...
		{
			CAMERASERIALPORT.setTimeout(1000);
			if (CAMERASERIALPORT.readBytes((char *)resp, 6) != 6)
			{
				continue;
			}
	...
	for (size_t x = PICT_OFFSET_X; x < PICT_OFFSET_X + pictureSize_X; x++)
	{
		dbg2("\n[x:", x)
			...
			for (size_t y = PICT_OFFSET_Y; y < PICT_OFFSET_Y + pictureSize_Y; y++)
			{
				...
				while (CAMERASERIALPORT.available() < 3) {
				...
				pixel_R = CAMERASERIALPORT.read();
				pixel_G = CAMERASERIALPORT.read();
				pixel_B = CAMERASERIALPORT.read();
				dbgHex(pixel_R)
					dbgHex(pixel_G)
					dbgHex(pixel_B)
...
void GetData_SD()  //versione che funziona con SD abilitata
{
	...
	if (false) {
		...
		retry:
			...
			clearRxBuf();
			sendCmd(cmd, 6);
			uint16_t cnt = CAMERASERIALPORT.readBytes((char *)pkt, PIC_PKT_LEN);
...
#pragma region SERIAL_CAMERA SETUP
	CAMERASERIALPORT.begin(115200);
	CAMERASERIALPORT.setTimeout(5000);
	SerialCamera_Sync();
#pragma endregion

	//initial(0xaa, 0x01, 0x00,0x03 , 0x01, 0x00);//raw non ricevo ack
 	//initial(0xaa, 0x01, 0x00, COLORSETTING_JPEG, 0x00, JPEGRESOLUTION_320X240); //così mi da l'ack
...
#define PICT_SIZE_Y 240 
 	...
 	//initial(0xaa, 0x01, 0x00, COLORSETTING_JPEG, 0, JPEGRESOLUTION_320X240); //così mi da l'ack
	SerialCamera_SetPktSize(PIC_PKT_LEN);//240x3
...
#define PICTURETYPE_JPEG 5
	...
	//GetPicture_toLCD(0xaa, 0x04, PICTURETYPE_SNAPSHOT, 0x00, 0x01, 0x00, PICT_SIZE_X, PICT_SIZE_Y);
 	...
 	SerialCamera_Initial(0xaa, 0x01, 0x00, COLORSETTING_JPEG, COLORSETTING_JPEG, JPEGRESOLUTION_320X240); //così mi da l'ack
