...
#define VSYNC 10      // Input Vertical Sync marking frame capture
#define PCLK  8      // Output FIFO buffer output clock
#define XCLK  7      // Output FIFO buffer output clock
...
#pragma region OV7670 REGISTERS AND SETTINGS
// da OV7670 FIFO
...
void InitializeOV7670Camera()
{
	...
	PulsePin(PCLK, DurationMicroSecs);
	PulsePin(PCLK, DurationMicroSecs);
...
void SetupOV7670ForQVGArgb565()// mio adattamanto
{
	...
	result = OV7670WriteReg(COM7, 0x04);
	...
	for (int height = 0; height < PHOTO_HEIGHT; height++)
	{
		for (int width = 0; width < PHOTO_WIDTH; width++)
		{
			for (int bytenumber = 0; bytenumber < PHOTO_BYTES_PER_PIXEL; bytenumber++)
			{
				...
				PinVal7 = ConvertPinValueToByteValue(digitalRead(D7_PIN), 7);
				PinVal6 = ConvertPinValueToByteValue(digitalRead(D6_PIN), 6);
				PinVal5 = ConvertPinValueToByteValue(digitalRead(D5_PIN), 5);
				PinVal4 = ConvertPinValueToByteValue(digitalRead(D4_PIN), 4);
				PinVal3 = ConvertPinValueToByteValue(digitalRead(D3_PIN), 3);
				PinVal2 = ConvertPinValueToByteValue(digitalRead(D2_PIN), 2);
				PinVal1 = ConvertPinValueToByteValue(digitalRead(D1_PIN), 1);
				PinVal0 = ConvertPinValueToByteValue(digitalRead(D0_PIN), 0);
...
#pragma region Mie aggiunte
volatile boolean onFrame = false;
volatile boolean onRow = false;


void ReadTransmitCapturedFrameToTFT()
{
	Serial.println(F("Starting Transmission of Photo To TFT ..."));
	// Read captured frame  and send each byte as it is read to TFT

	byte PixelDataH = 0;
	byte PixelDataL = 0;
	uint16_t pixelDataRGB565 = 0;
	byte PinVal7 = 0;
	byte PinVal6 = 0;
	byte PinVal5 = 0;
	byte PinVal4 = 0;
	...
	unsigned long  ByteCounter = 0;
	for (int x =PHOTO_HEIGHT ; x >0 ;x--)
	{
		onRow = false;
		while (!onRow) {
			PulsePin(XCLK, 1);
		}
		for (int y = 0; y < PHOTO_WIDTH; y++)
		{
			#if 0

				// ACQUISIZIONE PRIMO BYTE --------------------------
				// Pulse the read clock RCLK to bring in new byte of data.
				// 7ns for RCLK High Pulse Width and Low Pulse Width .007 micro secs
						PulsePin(PCLK, 1);
						...
						// Combine individual data from each pin into composite data in the form of a single byte
						PixelDataH = PinVal7 | PinVal6 | PinVal5 | PinVal4 | PinVal3 | PinVal2 | PinVal1 | PinVal0;
							...
							// Combine individual data from each pin into composite data in the form of a single byte
							PixelDataL = PinVal7 | PinVal6 | PinVal5 | PinVal4 | PinVal3 | PinVal2 | PinVal1 | PinVal0;
							pixelDataRGB565 = (PixelDataH << 8) | PixelDataL;
				...
				pixelDataRGB565 = ((byteRRRRRGGG << 8) | (byteGGGBBBBB));// 

				//Serial.print(".");
				//while ((REG_PIOD_PDSR & (1 << 10)));//wait for low
				//pixelDataRGB565 = (REG_PIOC_PDSR & 0xFF000) >> 6;
			...
			//pixelDataRGB565 = 0b0000011111100000;
			tft.drawPixel(x, y, pixelDataRGB565);
...
//	CaptureOV7670Frame();
	ReadTransmitCapturedFrameToTFT();
...
int OV7670Read(int start, byte *buffer, int size)
{
	...
	Wire.requestFrom(OV7670_I2C_READ_ADDRESS, size, true);
	i = 0;
...
#pragma endregion
#pragma region Presi da TestOV7670.ino
	#pragma region registri OV7670
	/* Registers */
...
void camHWInterfaceInit(void) {
	Serial.println(F("Entering camHWInterfaceInit...."));
	Serial.println(F("\tSetting 10.5MHz Clock...."));
...
void setup()
{
	...
	#pragma region ILI9341_due tft  setup
		 ...
		 tft.printAligned(F("OV7670FIFO"), gTextAlignMiddleCenter);
	...
	#pragma endregion
	 ...
	 //SetupOV7670ForVGARawRGB();
	 SetupOV7670ForQVGArgb565();
	 attachInterrupt(VSYNC, vsync_rising, RISING);		//onFrame = true;
	 attachInterrupt(HREF, href_rising, RISING);		//onRow = true;

#if 0
	 Serial.println(F("MIE PERSONALIZZAZIONI ..."));
	 int result;
	 String sresult;
	 result = OV7670WriteReg(SCALING_XSC, SCALING_XSC_VALUE_COLOR_BAR);
	 sresult = ParseI2CResult(result);
	 Serial.print(F("SCALING_XSC: "));
	 Serial.println(sresult);

	 result = OV7670WriteReg(REG_COM10, 32);//bit5=1:PCLK does not toggle on HBLANK., bit4=1: PCLK reverse ; bit1=1:VSYNC negative
	 sresult = ParseI2CResult(result);
	 Serial.print(F("REG_COM10: "));
	 Serial.println(sresult);
...
{   
	...
	{
		// Take Photo
		Serial.println(F("\nGoing to display camera snapshot on tft:"));
		DisplayCurrentCommand();
		...
		OV7670WriteReg(REG_COM10, 0x02);//VSYNK Negative
		...
		Serial.println(F("Photo Taken .. can you see? ..."));
...
byte yuv2rgb565(byte Y, byte U, byte V) {
	//yuv >rgb888
	byte r, g, b;
