...
int DecodeJpgFileData(const unsigned char* buf,                         const int sizeBuf,                                unsigned char** rgbpix,                            unsigned int* width,                                unsigned int* height);
...
#define dprintf(s,v) Serial.println(s,v)
#define dprintf(s) Serial.println(s,v)
...
int ConvertJpgFile(char* szJpgFileInName, char * szBmpFileOutName)
{
	...
	fp = SD.open(szJpgFileInName, FILE_WRITE);

	if ((myFile = SD.open(szJpgFileInName)) == NULL) {
...
#pragma region SD setup
	...
	if (!SD.begin(SD_CS)) {
		progmemPrintln(PSTR("sd init failed!"));
		return;
	}
