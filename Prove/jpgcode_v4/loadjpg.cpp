...
extern void Serial.print(const char *fmt, ...);

__forceinline void Serial.print(const char *fmt, ...) 
{
	va_list parms;
	char buf[256];
...
{
	Serial.print("# Decoded 8x8 Block#\n");
	for( int y=0; y<8; y++)
	{
		for( int x=0; x<8; x++)
		{
			Serial.print(  val[x][y]);
		}
		Serial.print("\n");
	}
}
...
inline void PrintSOF(const unsigned char *stream)
{
	...
	Serial.print("> SOF marker\n");
	Serial.print("Size:w x h \t nr_components:%d (%s)  precision: \n");
 
	Serial.print(width); Serial.print("x"); Serial.print(height);	Serial.print("\t");
	Serial.print(nr_components);	Serial.print("(");
	Serial.print(nr_components_to_string[nr_components]);	Serial.print(")\t");
	Serial.print(precision);	Serial.print("\n");
}
...
inline int ParseSOF(stJpegData *jdata, const unsigned char *stream)
{
	...
	*/
	...
	for (int i=0; i<nr_components; i++) 
	{
		...
		Serial.print("\nComponent"); Serial.print(cid);
		Serial.print("\t v factor:"); Serial.print(c->m_vFactor);
		Serial.print("\t h factor"); Serial.print(c->m_hFactor);
		Serial.print("\tQuantization table:"); Serial.println(Q_table);
	}
	jdata->m_width = width;
...
inline int ParseDQT(stJpegData *jdata, const unsigned char *stream)
{
	...
	while (length>0)
	{
		...
		if (qprecision)
		{
			...
			Serial.print("Error - 16 bits quantization table is not supported\n");
		}

		if (qindex>4)
		{
			Serial.print("Error - No more 4 quantization table is supported (got %d)");
			Serial.println( qi);
		}
...
inline int ParseSOS(stJpegData *jdata, const unsigned char *stream)
{
	...
	*/
	...
	Serial.print("> SOS marker\n");

	if (nr_components != 3)
	{
		Serial.print("Error - We only support YCbCr image\n");
	}
	...
	for (unsigned int i=0;i<nr_components;i++) 
	{
		...
		if ((table&0xf)>=4)
		{
			Serial.print("Error - We do not support more than 2 AC Huffman table\n");
		}
		if ((table>>4)>=4)
		{
			Serial.print("Error - We do not support more than 2 DC Huffman table\n");
		}
 		Serial.print("\nComponentId:"); Serial.print(cid);
		Serial.print(" tableAC:"); Serial.print(table & 0);
		Serial.print(" tableDC:"); Serial.println(table>>4);
...
inline int ParseDHT(stJpegData *jdata, const unsigned char *stream)
{
	...
	*/
	...
	Serial.print("> DHT marker length=");
	...
	while (length>0) 
	{
		...
		if (count > 256)
		{
			Serial.print("Error - No more than 1024 bytes is allowed to describe a huffman table");
		}
		if ( (index &0xf) >= HUFFMAN_TABLES)
		{
			Serial.print("Error - No more than  Huffman tables are supported: ");
				Serial.println(HUFFMAN_TABLES);
		}
//		Serial.print("Huffman table %s n%d\n", (index&0xf0)?"AC":"DC", index&0xf);
		Serial.print("Huffman table ");
		Serial.print((index & 0xf0) ? "AC" : "DC");
		Serial.print( index & 0xf);
		
		Serial.print("Length of the table: "); Serial.println( count);
...
inline int ParseJFIF(stJpegData *jdata, const unsigned char *stream)
{
	...
	while (!sos_marker_found)
	{
		...
		switch (marker)
		{
			...
			{
				Serial.print("DRI - Restart_marker\n");
			}
			break;

			case APP0:
			{
				Serial.print("APP0 Chunk ('txt' information) skipping\n");
			}
			break;

			default:
			{
				Serial.print("ERROR> Unknown marker "); Serial.print( marker);
			}
			break;
		}
	...
	if (!dht_marker_found) 
	{
		Serial.print("ERROR> No Huffman table loaded\n");
	}

	return 0;

	bogus_jpeg_format:
	Serial.print("ERROR> Bogus jpeg format\n");
	return -1;
}
...
inline int JpegParseHeader(stJpegData *jdata, const unsigned char *buf, unsigned int size)
{
	...
	Serial.print("-|- File thinks its size [bytes]:");
...
{
	Serial.print("HufCodes\n");
	Serial.print("Num: %d\n"); Serial.println( table->m_numBlocks);
	for (int i = 0; i<table->m_numBlocks; i++)
	{
		Serial.print(i); 
		Serial.print(" \t");
		Serial.println(IntToBinary(table->m_blocks[i].code, table->m_blocks[i].length));
	}
	Serial.print("\n");
...
void DumpDCTValues(short dct[64])
{
	...
	for (int i=0; i<64; i++)
	{
		Serial.print("  ");Serial.print(dct[c++]);

		if ( (c>0) && (c%8==0) ) Serial.print("\n");
	}
	Serial.print("\n");
}
...
//	DumpHufCodes(c->m_acTable);
	...
	if (!found)
	{
		Serial.print("-|- ##ERROR## We have a *serious* error, unable to find huffman code\n");
	}
		...
		for (k=1; k<=16; k++)
		{
			...
			if (IsInHuffmanCodes(code, k,  c->m_acTable->m_numBlocks, c->m_acTable->m_blocks, &decodedValue))
			{
				...
				else
				{
					...
					if (nr > 63)
					{
						Serial.print("-|- ##ERROR## Huffman Decoding\n");
					}
...
//			Serial.print("-[%d][%d][%d]-\t", poff, yoff, coff);
		}
//		Serial.print("\n");
	}
//	Serial.print("\n\n");
					  ...
					  unsigned int* height)		// Output image height
{
	...
	if (jdec == NULL)
	{
		Serial.print("Not enough memory to alloc the structure need for decompressing\n");
		return 0;
	}
...
int ConvertJpgFile(char* szJpgFileInName, char * szBmpFileOutName)
{
	...
	if (fp == NULL)
	{
		Serial.print("Cannot open jpg file:\n"); Serial.print(  szJpgFileInName);
		return 0;
	}
	...
	if (buf == NULL)
	{
		Serial.print("Not enough memory for loading file\n");
		return 0;
	}
	...
	if (rgbpix==NULL)
	{
		Serial.print("Failed to decode jpg\n");
		return 0;
	}
